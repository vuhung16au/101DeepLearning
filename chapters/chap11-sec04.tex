...
\begin{remark}[Data Splitting in Deep Learning]
...
\begin{definition}[Standardization (Z-score)]
...
\begin{example}[Imbalanced Data in Social Media]
Consider a social media platform with 1 million posts where 950,000 are normal content and only 50,000 are spam (5% spam rate). A naive classifier that always predicts "normal" would achieve 95% accuracy, but would miss all spam posts. This severe class imbalance makes it crucial to use techniques like class weighting, SMOTE, or focal loss to ensure the model learns to detect the minority class effectively.
\end{example}
...
\begin{remark}[Transfer Learning]
