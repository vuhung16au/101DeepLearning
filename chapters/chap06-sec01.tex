...
\begin{tikzpicture}[scale=0.7]
    ...
    \foreach \i in {1,2,3,4}
        \foreach \j in {1,2,3,4}
            \node[circle, draw, fill=bookpurple!20, minimum size=0.4cm] (x\i\j) at (\j*0.5, -\i*0.5) {};
    
    % Hidden layer
    \foreach \i in {1,2,3}
        \node[circle, draw, fill=bookpurple!40, minimum size=0.6cm] (h\i) at (3, -\i*0.8) {$h_\i$};
    
    % Output layer
    \node[circle, draw, fill=bookred!30, minimum size=0.8cm] (y3) at (5.5, -1.2) {3};
    \node[circle, draw, fill=bookred!30, minimum size=0.8cm] (y7) at (5.5, -2.4) {7};
    ...
    % Labels
    \node at (1, 0.5) {\textbf{Input: 4×4 pixels}};
    \node at (3, 0.5) {\textbf{Hidden Layer}};
    \node at (5.5, 0.5) {\textbf{Output: "3" or "7"}};
    
    % Decision arrows
    \draw[->, bookred, thick] (y3) -- (6.5, -1.2) node[right] {Confidence: 0.8};
    \draw[->, bookred, thick] (y7) -- (6.5, -2.4) node[right] {Confidence: 0.2};
\end{tikzpicture}
\caption{Simple neural network for digit classification: "This is a 3" or "This is a 7". The network learns to distinguish between these two digits by analyzing pixel patterns.}
\label{fig:digit-classification-nn}
\end{figure}
...
\index{multilayer perceptrons (MLPs)}
\index{activation functions}
\index{loss functions}
